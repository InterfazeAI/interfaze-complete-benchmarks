"""
OlmOCR Benchmark for Extend.ai /parse_runs.

Mirrors olmocr_bench_reducto.py — same dataset, same per-page contract, just
swaps Reducto for Extend so candidate outputs land under data_dir/extend/.

Usage:
    uv run -m benchmarks.olmocr.olmocr_bench_extend
    uv run -m benchmarks.olmocr.olmocr_bench_extend --sample
    uv run -m benchmarks.olmocr.olmocr_bench_extend --skip-generation
    uv run -m benchmarks.olmocr.olmocr_bench_extend --generate-only
"""
...
USAGE_OUTPUT = RESULTS_DIR / "olmocr_extend_v2_usage.json"
CANDIDATE_NAME = "extend_v2"
RATE_LIMIT = 8
MAX_RETRIES = 3
...
async def process_page(pdf_path, page_num, output_path, rate_limiter):
    from olmocr.bench.runners.run_extend import run_extend

    for attempt in range(MAX_RETRIES):
        await rate_limiter.acquire()
        try:
            result = await asyncio.to_thread(run_extend, pdf_path, page_num)
...
async def generate_outputs(data_dir: Path):
    ...
    try:
        from src.commons_extend import write_usage_snapshot
