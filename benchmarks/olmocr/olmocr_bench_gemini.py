"""
OlmOCR Benchmark for Gemini models (native google-genai, default 3.7 Flash).

Mirrors olmocr_bench.py but routes pages through the native Gemini API, so
the methodology matches the repo's other --provider gemini runs (thinking_level
rather than OpenRouter's reasoning.effort).

Usage:
    uv run -m benchmarks.olmocr.olmocr_bench_gemini
    uv run -m benchmarks.olmocr.olmocr_bench_gemini --sample
    uv run -m benchmarks.olmocr.olmocr_bench_gemini --skip-generation
    uv run -m benchmarks.olmocr.olmocr_bench_gemini --generate-only
    uv run -m benchmarks.olmocr.olmocr_bench_gemini --model accounts/fireworks/models/inkling-small
"""
...
CANDIDATE_NAME = "gemini-3-7-flash"
RATE_LIMIT = 25
MAX_RETRIES = 3
...
# queue server-side and eventually get rate-limited. Override with
# BENCH_MAX_IN_FLIGHT.
...
async def _process_page(pdf_path, page_num, output_path, rate_limiter):
    from olmocr.bench.runners.run_gemini import run_gemini

    for attempt in range(MAX_RETRIES):
        await rate_limiter.acquire()
        try:
            result = await asyncio.to_thread(run_gemini, pdf_path, page_num, MODEL)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run OlmOCR benchmark against a Gemini model (native API)"
    )
    parser.add_argument(
        "--model", default=DEFAULT_MODEL, help="Gemini model id (e.g. gemini-3.7-flash)"
    )
