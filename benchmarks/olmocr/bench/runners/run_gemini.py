...
def _get_client():
    ...
    if _client is None:
        ...
        # Cached deliberately: building a client per call gets it collected
        # mid-flight ("Cannot send a request, as the client has been closed").
        _client = genai.Client(api_key=api_key)
    return _client


def _thinking_floor(model: str) -> str:
    """Lowest thinking_level the model accepts.

    gemini-3.7+ flash rejects MINIMAL server-side ("Thinking level MINIMAL is
    not supported for this model") and the SDK enum has no off/none, so "low" is
    its floor. Older 3.x flash still takes "minimal"; Pro models reject
    "minimal" too. Override with BENCH_GEMINI_FLOOR.
    """
    override = os.getenv("BENCH_GEMINI_FLOOR")
    if override:
        return override
    m = model.lower()
    if "pro" in m or any(v in m for v in ("3.7", "3.8", "3.9", "4.")):
        return "low"
    return "minimal"
...
def run_gemini(
    ...
    """Convert a PDF page to markdown via any vision-capable Gemini model.

    Generalises run_gemini_pro_31, which pinned both the model and its thinking
    config. Same prompt, so scores stay comparable across models.
    """
    ...
    config = types.GenerateContentConfig(
        temperature=0.0,
        thinking_config=types.ThinkingConfig(thinking_level=_thinking_floor(model)),
    )
