...
def _get_client() -> OpenAI:
    """Host is chosen by BENCH_OLMOCR_PROVIDER (fireworks|openrouter). The hosts
    are not equivalent at the same nominal reasoning setting — see README."""
    global _client
    if _client is None:
        if os.getenv("BENCH_OLMOCR_PROVIDER") == "openrouter":
        ...
        if not api_key:
            raise SystemExit(
                "FIREWORKS_API_KEY not set — get it from "
...
def run_fireworks(
    ...
    """Convert a PDF page to markdown via a Fireworks-hosted model (Inkling).

    reasoning_effort='none' is Inkling's floor — it still thinks, so the page
    transcription is preceded by reasoning tokens we pay for but discard."""
    ...
    response = client.chat.completions.create(
        ...
        max_tokens=20000,
        **(
            {"extra_body": {"reasoning": {"effort": os.getenv("BENCH_OLMOCR_EFFORT", "none")}}}
            if os.getenv("BENCH_OLMOCR_PROVIDER") == "openrouter"
            else {"reasoning_effort": os.getenv("BENCH_OLMOCR_EFFORT", "none")}
        ),
    )
